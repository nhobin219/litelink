...
def main() -> None:
    ...
    parser.add_argument("--rate", type=float, default=2000.0, help="trades per second")
    ...
    print(f"capturing {NAME} into {args.root} at ~{args.rate:,.0f} trades/s")
    ...
    try:
        for tick in itertools.count():
            ...
            if tick % 50 == 0:
                ...
                print(
                    f"  {written:>9,} trades  {written / max(elapsed, 1e-9):>7,.0f}/s"
    ...
    print(f"{written:,} trades appended, all durable at commit")
